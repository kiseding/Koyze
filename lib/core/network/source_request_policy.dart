  ...
  String? singleHeader(String name) {
    ...
    if (values.length != 1) {
      throw const SourceRequestPolicyException(
        'ambiguous_redirect',
        'Redirect has multiple Location values',
      );
    }
...
class SourceRequestPolicy {
  ...
  /// Deprecated compatibility flag. Non-public destinations are always
  /// rejected; user-provided scripts are not a network security boundary.
  ...
  Future<ValidatedSourceRequest> validate(
    ...
    if (options['httpsOnly'] == true && scheme != 'https') {
      throw const SourceRequestPolicyException(
        'scheme',
    ...
    if (publicAddresses.length != addresses.length || publicAddresses.isEmpty) {
      throw const SourceRequestPolicyException(
        'blocked_address',
        'Destination must resolve only to public addresses',
      );
    }
    // Always connect only to the validated public addresses. The compatibility
    // flag is intentionally ignored so a caller cannot weaken SSRF checks.
  ...
  static bool _isPublic(InternetAddress address) {
    ...
    if (address.type == InternetAddressType.IPv4) {
      return !_ipv4DeniedPrefixes.any(
        (prefix) => _matchesPrefix(bytes, prefix.$1, prefix.$2),
      );
    }
    final globallyRoutable = (bytes[0] & 0xe0) == 0x20;
    return globallyRoutable &&
        !_ipv6DeniedPrefixes.any(
          (prefix) => _matchesPrefix(bytes, prefix.$1, prefix.$2),
        );
  }
...
typedef SourceTransport =
    Future<SourceTransportResponse> Function(
    ...
    try {
      for (var redirects = 0; ; redirects++) {
        ...
        final request = await Future.any([
          ...
          cancel.future.then<ValidatedSourceRequest>(
        ...
        final response = await Future.any([
          ...
          cancel.future.then<SourceTransportResponse>(
            (reason) => throw SourceRequestPolicyException('cancelled', reason),
          ),
        ]);
        ...
        if (isRedirect) {
          ...
          try {
            ...
            currentOptions = sourceRedirectOptions(
              request,
              redirectStatus,
    ...
    subscription = response.body.listen(
      (chunk) {
        ...
        if (chunk.length > policy.maximumResponseBytes - bytes.length) {
          completeError(
            const SourceRequestPolicyException(
              'response_too_large',
...
Map<String, dynamic> sourceRedirectOptions(
  ValidatedSourceRequest request,
  int status, {
  ...
  for (final entry in request.headers.entries) {
    ...
    if (crossOrigin && !_crossOriginSafeHeaders.contains(lower)) continue;
  ...
  return {
    ...
    if (request.httpsOnly) 'httpsOnly': true,
  };
}

class SourceStreamSandbox {
  final SourceRequestPolicy policy;
  final SourceTransport transport;
  final int maximumRedirects;

  const SourceStreamSandbox({
    required this.policy,
    required this.transport,
    this.maximumRedirects = 5,
  });

  Future<SourceStreamResponse> open(
    Uri uri,
    Map<String, dynamic> options, {
    SourceRequestCancellation? cancellation,
  }) async {
    ...
    var currentOptions = Map<String, dynamic>.from(options);
    for (var redirects = 0; ; redirects++) {
      if (cancel.isCancelled) _throwSourceCancelled();
      final request = await policy.validate(current, currentOptions);
      final response = await transport(request, cancel);
      if (cancel.isCancelled) {
        response.close();
        _throwSourceCancelled();
      }
      final status = response.statusCode;
      final isRedirect =
          status == 301 ||
      ...
      try {
        ...
        currentOptions = sourceRedirectOptions(
          request,
          status!,
          crossOrigin: !_sameSourceOrigin(current, next),
        );
        current = next;
      } finally {
        response.close();
...
bool _sameSourceOrigin(Uri left, Uri right) =>
    left.scheme.toLowerCase() == right.scheme.toLowerCase() &&
    left.host.toLowerCase() == right.host.toLowerCase() &&
    left.port == right.port;

Never _throwSourceCancelled() => throw const SourceRequestPolicyException(
  'cancelled',
  'Source request was cancelled',
);
