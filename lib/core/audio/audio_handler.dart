...
class LxAudioHandler extends BaseAudioHandler with QueueHandler, SeekHandler {
  ...
  Timer? _completionRecoveryTimer;
  int _completionRecoveryGeneration = -1;
  int _completionRecoveryAttempts = 0;
  ...
  static const Duration _outputRouteRecoveryMinimumProgress = Duration(
    milliseconds: 120,
  );
  ...
  static const _successfulPlaybackProgress = Duration(milliseconds: 500);
  ...
  @visibleForTesting
  int? get debugPreparedShuffleNextIndex => _preparedShuffleNextIndex;

  @visibleForTesting
  int get debugConsecutiveAutomaticTrackFailures =>
      _consecutiveAutomaticTrackFailures;

  @visibleForTesting
  bool get debugAutomaticFailureSkippingHalted =>
      _automaticFailureSkippingHalted;
  ...
  int _bumpGeneration() {
    _completionRecoveryTimer?.cancel();
    _completionRecoveryTimer = null;
    _completionRecoveryGeneration = -1;
    _completionRecoveryAttempts = 0;
  ...
  void _resetAutomaticTrackFailureChain() {
    _consecutiveAutomaticTrackFailures = 0;
  ...
  void _recordSuccessfulPlayback() {
    ...
    if (_automaticFailureSkippingHalted ||
        occurrenceId == null ||
        !_commands.installedSourceIsAuthoritative ||
        _installedPlaybackGeneration != _playGeneration ||
        _installedMediaId != _activeItemId ||
        _player.position < _successfulPlaybackProgress ||
        !_player.playing ||
  ...
  Future<void> _advanceAfterAuthoritativeTrackFailure({
    required int occurrenceId,
    ...
    if (_lastCountedFailedOccurrenceId == occurrenceId) return;
    _lastCountedFailedOccurrenceId = occurrenceId;
    _consecutiveAutomaticTrackFailures++;
    if (_consecutiveAutomaticTrackFailures >= _maximumConsecutiveFailedTracks) {
      _automaticFailureSkippingHalted = true;
      _userWantsPlay = false;
      final haltIntentGeneration = _userIntentGeneration;
      try {
        await _commands.setDesiredPlayingPreservingIntent(false);
      } catch (error, stackTrace) {
        AppLog.instance.record(
          'audio.failure_limit',
          'failed to halt native playback intent: $error',
          level: AppLogLevel.error,
          stackTrace: stackTrace,
        );
      }
      if (_disposed ||
          !_automaticFailureSkippingHalted ||
          occurrenceId != _activeOccurrenceId ||
          haltIntentGeneration != _userIntentGeneration) {
        return;
      }
      _publishPlaybackState(
      ...
      onError?.call('连续 5 首歌曲播放失败，已停止自动播放');
      return;
    }
    await _skipToNextInternal(seamless: true, provenance: provenance);
  }
  ...
  /// 把 just_audio 解析出的真实时长回填到系统媒体信息。
  /// iOS 锁屏系统卡片的进度条与歌曲时间依赖
  /// MPMediaItemPropertyPlaybackDuration > 0；搜索接口的 duration 字段
  /// 可能缺失（0），若不回填，锁屏将始终不显示歌曲时间。
  /// 仅在源安装完成、播放事务稳定（authoritative）后才回填，避免在
  /// resolution/play 事务进行中替换 mediaItem 破坏对象同一性校验。
  void _publishKnownDurationIfChanged() {
    final item = mediaItem.value;
    final playerDuration = _player.duration;
    if (item == null ||
        playerDuration == null ||
        playerDuration <= Duration.zero ||
        playerDuration == item.duration ||
        !_commands.installedSourceIsAuthoritative ||
        _installedPlaybackGeneration != _playGeneration ||
        _installedMediaId != _activeItemId) {
      return;
    ...
    final updated = item.copyWith(duration: playerDuration);
    _replaceQueueItem(index, updated);
  ...
  void _validateOutputRouteRecovery(Duration recoveryPosition) {
    ...
    _outputRouteRecoveryValidationGeneration = generation;
    _outputRouteRecoveryValidationUserIntentGeneration = userIntentGeneration;
    validation.ignore();
  }
  ...
  Future<void> _replaceNativePlayer() async {
    ...
    final oldDuration = _durationSubscription;
    final oldProcessingStates = _processingStateSubscription;
    AppLog.instance.record(
      'audio.recovery',
      'disposing previous native player',
    );
    ...
    AppLog.instance.record(
      'audio.recovery',
      'native player reconstruction completed',
    );
  }
  ...
  void _onTrackCompleted() {
    ...
    if (mayContinue) {
      // Keep iOS Now Playing alive while the next source is resolved. Without
      // this short buffering state, a completed native player may end the
      // background audio session before the next item is installed.
      _publishPlaybackState(
  ...
  void _scheduleCompletionRecovery({
    ...
    if (_disposed || !_userWantsPlay || interruptionActive) return;
    if (_completionRecoveryGeneration != generation) {
      _completionRecoveryGeneration = generation;
      _completionRecoveryAttempts = 0;
    }
    if (_completionRecoveryAttempts >= _maximumCompletionRecoveryAttempts) {
      AppLog.instance.record(
        'audio.autonext',
        'auto-next recovery exhausted item=$itemId generation=$generation',
        level: AppLogLevel.error,
      );
      onError?.call('自动播放下一首失败，请返回应用重试');
      return;
    }
    _completionRecoveryAttempts++;
    _completionRecoveryTimer?.cancel();
    AppLog.instance.record(
      'audio.autonext',
      'scheduling recovery attempt=$_completionRecoveryAttempts '
          'item=$itemId generation=$generation',
      level: AppLogLevel.warning,
    );
    _completionRecoveryTimer = Timer(_completionRecoveryDelay, () {
      if (_disposed ||
          generation != _playGeneration ||
          _activeOccurrenceId != occurrenceId ||
          _activeItemId != itemId ||
      ...
      if (_lastHandledCompletionGeneration == generation) {
        _lastHandledCompletionGeneration = -1;
      }
      _onTrackCompleted();
    });
    ...
    Future(() async {
      ...
      try {
        ...
        if (retryCurrent) {
          ...
          await _advanceAfterAuthoritativeTrackFailure(
            occurrenceId: occurrenceId,
            provenance: provenance,
          );
        }
  ...
  Future<void> _play() async {
    if (_disposed) return;
    _resetAutomaticTrackFailureChain();
    AppLog.instance.record(
      'audio.command',
    ...
    final selectionTransferPending =
  ...
  void _markOutputRouteRecoveryPending(String reason, {bool force = false}) {
  ...
  Future<void> _skipToNextInternal({
    ...
    final keepaliveInstalled = _useSilenceKeepalive
        ? await _commands.installTemporarySource(
            sourceCommandToken,
  ...
  Future<void> _dispose() async {
    ...
    if (durationSubscription != null) {
      await cleanup(durationSubscription.cancel);
    }
  ...
  Future<void> _setPlaylist(
    ...
    if (_disposed) return;
    if (userInitiated) _resetAutomaticTrackFailureChain();
    _clearOutputRouteRecovery();
    if (_disposed) return;
    final provenance = _captureStartProvenance();
