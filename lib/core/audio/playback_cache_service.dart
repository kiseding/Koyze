...
import '../network/source_media_transport.dart';
...
const _fileProtectionChannel = MethodChannel('koyze/file_protection');
...
class PlaybackUrlResolver<T> {
  ...
  static const _qualityChain = ['hires', 'flac24bit', 'flac', '320k', '128k'];
...
class PlaybackCacheService {
  ...
  PlaybackCacheService({
    ...
    this.maximumEntryBytes = 512 * 1024 * 1024,
  ...
  Future<void> dispose() {
    ...
    return _disposeFuture = _disposeAfterDrain(operations);
  }

  Future<void> _disposeAfterDrain(
    List<_InflightOperation> cancelledOperations,
  ) async {
    try {
      await _drainForDispose(cancelledOperations);
    } finally {
  ...
  Future<PlaybackCacheLease?> acquireOrDownload({
    ...
    try {
      return await acquireOrDownloadForResolution(
        ...
        songId: songId,
        quality: quality,
        fromCustomSource: fromCustomSource,
      );
    } on PlaybackCacheTerminalHttpException {
      return null;
    }
  ...
  Future<bool> validateStream({
    ...
    try {
      ...
      if (!CancelToken.isCancel(error)) {
        debugPrint(
          '[PlaybackCache] stream validation failed '
      ...
      debugPrint(
        '[PlaybackCache] stream validation error type=${error.runtimeType}',
      );
      return false;
    }
  }
  ...
  Future<String?> _getOrDownloadPath({
    ...
    required String songId,
    required String quality,
    required bool fromCustomSource,
  }) async {
    if (_disposed) return null;
    await init();
    if (_disposed) return null;
  ...
  Future<String?> _finalizeOperation(
    ...
    try {
      ...
      debugPrint(
        '[PlaybackCache] operation failed key=${operation.key} '
    ...
    try {
      ...
      if (_downloader != null) {
        ...
        await _sourceMediaTransport.download(
          ...
          cancelToken: token,
          byteLimit: maximumEntryBytes < maxBytes
              ? maximumEntryBytes
              : maxBytes,
        );
      } else {
      ...
      if (length > maximumEntryBytes ||
          length > maxBytes ||
          length == 0 ||
          length < (_downloader != null ? 16 : 2048)) {
      ...
      if (!CancelToken.isCancel(error)) {
        debugPrint(
          '[PlaybackCache] download failed key=$key '
          'type=${error.type.name} status=${error.response?.statusCode}',
        );
      }
      ...
      if (error.statusCode == 403 || error.statusCode == 404) {
        throw PlaybackCacheTerminalHttpException(error.statusCode);
      }
      return null;
    } catch (error) {
      debugPrint(
        '[PlaybackCache] download error key=$key type=${error.runtimeType}',
      );
      await _withKeyTransaction(
        key,
        () => _cleanupTransientFilesLocked(partPath, stagePath),
  ...
  Future<void> _downloadBoundedWithDio(
    ...
    final response = await _dio.get<ResponseBody>(
      ...
      cancelToken: token,
      options: Options(
        headers: mediaRequestHeaders(url, platform),
    ...
    final ceiling = maximumEntryBytes < maxBytes ? maximumEntryBytes : maxBytes;
