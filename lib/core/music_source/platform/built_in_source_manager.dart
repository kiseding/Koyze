...
class BuiltInSourceManager {
  ...
  Future<String?> getArtwork(String platformId, MusicItem music) async {
    final platform = _platforms[platformId];
    if (platform == null) return null;
    return platform.getArtwork(music);
  }
  ...
  Future<List<MusicItem>> getLeaderboardSongs(
    ...
    final platform = _platforms[platformId];
    if (platform == null) return [];
    return platform.getLeaderboardSongs(
      leaderboardId,
      page: page,
      limit: limit,
    );
  ...
  Future<List<LeaderboardCategory>> getAllLeaderboardCategories() async {
    ...
    final results = await Future.wait(
      _platforms.values.map((p) async {
        try {
          return await p.getLeaderboardCategories().timeout(
