...
String normalizeKuwoArtwork(Map<String, dynamic> item) {
  for (final key in ['pic', 'artistPic', 'img300', 'img']) {
    final value = item[key]?.toString().trim() ?? '';
    if (value.isNotEmpty) return normalizeNeteaseArtwork(value);
  }

  final albumPath = item['web_albumpic_short']?.toString().trim() ?? '';
  ...
  if (artistPath.isNotEmpty) {
    final normalized = artistPath.replaceFirst(RegExp(r'^120/'), '500/');
    return normalized.startsWith('http')
        ? normalizeNeteaseArtwork(normalized)
        : 'https://img1.kuwo.cn/star/starheads/$normalized';
  }
  return '';
}
