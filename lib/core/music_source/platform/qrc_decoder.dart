...
String decryptQrc(String hex) {
  ...
  if (encrypted.isEmpty || encrypted.length % 8 != 0) {
    throw const FormatException('QRC data length must be a multiple of 8');
  }
...
Uint8List _hexToBytes(String hex) {
  ...
  if (normalized.length % 2 != 0) {
    throw const FormatException('QRC hex string must have even length');
  }
...
Int32List _buildSpTable() {
  ...
  for (var sBoxIdx = 0; sBoxIdx < 8; sBoxIdx++) {
    for (var sBoxInput = 0; sBoxInput < 64; sBoxInput++) {
      ...
      table[(sBoxIdx << 6) | sBoxInput] = _applyQqPboxPermutation(prePBoxVal);
    }
  }
  return table;
}
...
void _desCrypt(
  ...
  for (var i = 0; i < 15; i++) {
    final temp = right;
    right =
        (left ^ _fFunction(right, schedule0[i * 2], schedule0[i * 2 + 1])) &
        0xFFFFFFFF;
    left = temp;
  }
  ...
  for (var i = 0; i < 15; i++) {
    final temp = right;
    right =
        (left ^ _fFunction(right, schedule1[i * 2], schedule1[i * 2 + 1])) &
        0xFFFFFFFF;
    left = temp;
  }
  ...
  for (var i = 0; i < 15; i++) {
    final temp = right;
    right =
        (left ^ _fFunction(right, schedule2[i * 2], schedule2[i * 2 + 1])) &
        0xFFFFFFFF;
    left = temp;
  }
