...
class KwSource extends MusicPlatform {
  ...
  Future<String?> getArtwork(MusicItem music) async {
    ...
    try {
      ...
      if (url.startsWith('http')) {
        return normalizeNeteaseArtwork(url);
      }
      return null;
    } catch (e) {
      return null;
      ...
      return abslist
          .map((item) {
            ...
            return MusicItem(
              ...
              source: 'kw',
              platform: 'kw',
              artwork: normalizeKuwoArtwork(m),
              album: '${(m['songnum'] ?? 0)} 首',
              isPlayable: false,
            );
          })
  ...
  Future<List<LeaderboardCategory>> getLeaderboardCategories() async {
    ...
    // 并发池限制：41 个榜单全部并发会打请求风暴，改为最多 6 路并发
    return _withConcurrency(fallback, 6, (c) async {
      try {
  ...
  static Future<List<T>> _withConcurrency<T>(
    List<T> items,
    int concurrency,
    Future<T> Function(T item) task,
  ) async {
    if (items.isEmpty) return items;
    final results = List<T?>.filled(items.length, null, growable: false);
    var nextIndex = 0;
