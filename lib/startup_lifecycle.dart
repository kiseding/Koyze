...
enum StartupBootstrapPhase { loading, ready, failed }

final class StartupBootstrapState {
  const StartupBootstrapState.loading()
    : phase = StartupBootstrapPhase.loading,
      error = null,
      child = null;
  const StartupBootstrapState.ready(this.child)
    : phase = StartupBootstrapPhase.ready,
      error = null;
  const StartupBootstrapState.failed(this.error)
    : phase = StartupBootstrapPhase.failed,
      child = null;

  final StartupBootstrapPhase phase;
  final Object? error;
  final Widget? child;
}

class StartupBootstrapGate extends StatelessWidget {
  ...
  final ValueNotifier<StartupBootstrapState> status;
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder<StartupBootstrapState>(
      ...
      builder: (context, state, _) {
        if (state.phase == StartupBootstrapPhase.ready && state.child != null) {
          return state.child!;
        }
        return MaterialApp(
          ...
          home: _StartupStatusPage(state: state, onRetry: onRetry),
        );
      },
    );
  }
...
class _StartupStatusPage extends StatelessWidget {
  const _StartupStatusPage({required this.state, required this.onRetry});
  ...
  @override
  Widget build(BuildContext context) {
    final failed = state.phase == StartupBootstrapPhase.failed;
    return Scaffold(
      body: Center(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Icon(
                failed ? Icons.warning_amber_rounded : Icons.graphic_eq_rounded,
                size: 48,
                color: failed
...
class _OwnedProviderScopeState extends State<OwnedProviderScope> {
  ...
  void dispose() {
    unawaited(
      widget.lifecycle.dispose().catchError((error, stackTrace) {
        FlutterError.reportError(
          FlutterErrorDetails(
            ...
            context: ErrorDescription(
              'while disposing the root provider scope',
            ),
          ),
        );
      }),
