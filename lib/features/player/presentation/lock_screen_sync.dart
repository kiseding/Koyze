...
const String _appGroupId = 'group.com.koyze.app';
const String _widgetKind = 'KoyzeHomeWidget';

({bool playing, AudioProcessingState processingState, int? queueIndex})
lockScreenPlaybackSnapshot(PlaybackState state) => (
...
class LockScreenSyncService {
  ...
  late final SerializedSyncCoordinator _syncCoordinator =
      SerializedSyncCoordinator(_syncGeneration);
  ...
  Future<void> _init() async {
    ...
    _subscriptions.add(
      _handler.playbackState
          .map(lockScreenPlaybackSnapshot)
          .distinct()
          .listen((_) => _syncNow()),
    );
    _subscriptions.add(
  ...
  Future<void> _syncNow() =>
      _disposed ? Future<void>.value() : _syncCoordinator.request();

  Future<void> _syncGeneration(int generation) async {
    if (_disposed || !_syncCoordinator.owns(generation)) return;
    try {
      final item = _handler.mediaItem.value;
      if (item == null) {
        await _clearNowPlaying(generation);
        return;
      }
      final playback = _handler.playbackState.value;
      final transitioning =
          playback.processingState == AudioProcessingState.completed &&
          playback.playing;
      final playing = playback.playing || transitioning;
      ...
      final positionSyncedAtMs = DateTime.now().millisecondsSinceEpoch
          .toDouble();
      _lastLyric = lyric;
      ...
      await _saveArtwork(music, generation);
    } catch (e) {
  ...
  Future<void> _syncTick() async {
    ...
    try {
      ...
      if (!lyricChanged && _positionTick % 10 != 0) {
        _positionTick++;
        return;
      }
      _positionTick++;
  ...
  Future<void> _clearNowPlaying(int generation) async {
    _lastMusicKey = null;
    _artworkPath = null;
    _lastLyric = null;
  ...
  Future<void> _saveArtwork(MusicItem music, int generation) async {
    ...
    try {
      ...
      if (directory == null || !_syncCoordinator.owns(generation)) return;
      final artworkFile = File(
        '${directory.path}/home_widget/artwork-$generation.jpg',
      );
...
@visibleForTesting
class SerializedSyncCoordinator {
  SerializedSyncCoordinator(this._sync);

  final Future<void> Function(int generation) _sync;
  int _generation = 0;
  bool _dirty = false;
  bool _disposed = false;
  Future<void>? _active;
