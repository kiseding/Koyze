...
final class PlaylistSnapshot {
  ...
  static MusicItem _copySong(MusicItem song, String metaPath) {
    return MusicItem(
      ...
      hash: song.hash,
      meta: song.meta == null ? null : _copyJsonObject(song.meta!, metaPath),
...
final class PlaylistSnapshotCodec {
  ...
  Playlist _decodePlaylist(dynamic value, String path, Set<String> ids) {
  ...
  MusicItem _decodeSong(dynamic value, String path) {
    ...
    final id = _nonEmpty(_string(json['id'], '$path.id'), '$path.id');
    final name = _nonEmpty(_string(json['name'], '$path.name'), '$path.name');
    final singer = _nonEmpty(
      _string(json['singer'], '$path.singer'),
      '$path.singer',
    );
    final source = _nonEmpty(
      _string(json['source'], '$path.source'),
      '$path.source',
    );
    ...
    _copyOptionalString(json, validated, 'playlistItemId', path);
