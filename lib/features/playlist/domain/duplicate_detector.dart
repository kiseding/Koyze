...
class DuplicateDetector {
  DuplicateDetector({required this.songs, required this.favoriteIds});
  ...
  List<DuplicateGroup> detect() {
    ...
    for (final song in songs) {
      final key = _DuplicateKey(
        ...
        durationBucket: (song.duration.inSeconds / durationBucketSec).floor(),
      );
  ...
  int _qualityScore(MusicItem song) {
    var score = 0;
    if (favoriteIds.contains(song.identityKey)) score += 1000;
