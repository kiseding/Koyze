...
class RecommendationEngine {
  ...
  Map<String, double> _normalize(Map<String, int> counts) {
    ...
    return {for (final entry in counts.entries) entry.key: entry.value / max};
  }
  ...
  /// 排除已收藏歌曲后返回最多 30 首推荐；所有候选均已被收藏时返回空列表。
  List<RecommendedSong> recommend({
    ...
    final favoriteIds = favorites.map((s) => s.identityKey).toSet();
    final favoriteKeys = favorites
        .map(
          (s) =>
              '${s.name.trim().toLowerCase()}|${s.singer.trim().toLowerCase()}',
        )
        .toSet();
