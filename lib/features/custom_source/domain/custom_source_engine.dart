...
import 'source_zlib.dart';
import 'source_script_safety.dart' as script_safety;
...
const String customSourceExecutionUnavailableReason =
    'Custom source execution was rejected by the synchronous safety preflight.';

// Ordinary sources remain usable. The engine still cannot guarantee an
// interrupt for arbitrary hostile JavaScript, so known synchronous hazards are
// rejected before the native runtime evaluates the script.
bool get isCustomSourceExecutionSupported => true;
...
class CustomSourceEngine {
  ...
  void _setupBaseEnvironment() {
    ...
    _runtime!.onMessage('lx_zlib', (dynamic args) async {
      try {
        return await runBoundedSourceZlib(args);
      } catch (_) {
  ...
  Future<bool> loadSource(CustomSource source) async {
    // Do not evaluate known synchronous hazards in the native runtime. A Dart
    // timeout cannot interrupt JavaScriptCore/QuickJS once evaluation starts.
    if (script_safety.hasUnsafeSynchronousLoop(source.script)) {
      _currentSource = source;
      _emitError(customSourceExecutionUnavailableReason);
      return false;
    }
  ...
  Future<void> _drainDeferredMessages() async {
    while (_deferredMessages.isNotEmpty) {
      ...
      if (channel == 'lx_send') {
        ...
        final result = await _runZlib(payload);
  ...
  Future<String> _runZlib(dynamic args) async {
    try {
      return await runBoundedSourceZlib(args);
    } catch (_) {
