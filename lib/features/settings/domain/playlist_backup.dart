...
BackupData decodeBackup(
  ...
  final additionalSettings = _decodeAdditionalSettings(
    root['additional_settings'],
  );
...
Map<String, dynamic> _decodeAdditionalSettings(Object? raw) {
  if (raw == null) return const {};
  if (raw is! Map) {
    throw const FormatException('additional_settings must be an object');
  }
...
final class BackupRestoreCoordinator {
  ...
  Future<void> restore(BackupData data) async {
    ...
    try {
      ...
      for (final entry in data.additionalSettings.entries) {
        await _writeAdditionalSetting(entry.key, entry.value);
      }
  ...
  Future<void> _writeAdditionalSetting(String key, Object? value) async {
    if (value == null) {
      ...
      throw FormatException('unsupported additional setting: $key');
    }
  }
}
