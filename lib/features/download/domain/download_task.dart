...
class DownloadTask {
  ...
  String get musicIdentityKey => MusicItem(
    id: musicId,
    name: name,
    singer: singer,
    source: source ?? '',
  ...
  static DownloadTask decodePersisted(Object? raw) {
    ...
    return DownloadTask(
      ...
      name: name,
      singer: singer,
      // Remote media links are short-lived bearer capabilities. Legacy values
      // remain readable, but are deliberately discarded at this boundary.
      url: null,
  ...
  static DateTime _requireDate(Map<String, dynamic> json, String key) {
    ...
    if (value is! String) {
      throw FormatException(
        'expected date String for $key, got ${value.runtimeType}',
      );
    }
    try {
