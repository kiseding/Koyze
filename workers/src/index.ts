/**
 * koyze-api — Cloudflare Worker for Koyze
 *
...
import { handleSyncStage, handleSyncAbort, handleSyncCommit, handleSyncStateGet, handleSyncStateSet } from './routes/user/sync';
import { handleSyncAccountStatus, handleSyncPull, handleSyncPush, handleSyncSnapshot } from './routes/user/sync-events';
...
const routes = new Map<string, RouteHandler>([
  ...
  ['GET/api/user/sync/status', (req, _url, env) => handleUserSyncStatus(req, env)],
  ['POST/api/user/sync/stage', (req, _url, env) => handleSyncStage(req, env)],
  ['DELETE/api/user/sync/stage', (req, url, env) => handleSyncAbort(req, url, env)],
  ['POST/api/user/sync/commit', (req, _url, env) => handleSyncCommit(req, env)],
  ['GET/api/user/sync/state', (req, _url, env) => handleSyncStateGet(req, env)],
  ['POST/api/user/sync/state', (req, _url, env) => handleSyncStateSet(req, env)],
  ['POST/api/sync/push', (req, _url, env) => handleSyncPush(req, env)],
  ['GET/api/sync/pull', (req, url, env) => handleSyncPull(req, url, env)],
  ['GET/api/sync/full', (req, _url, env) => handleSyncPull(req, new URL(`${req.url}?cursor=0`), env)],
  ['GET/api/sync/account-status', (req, _url, env) => handleSyncAccountStatus(req, env)],
  ['GET/api/sync/snapshot', (req, _url, env) => handleSyncSnapshot(req, env)],
  ...
  ['GET/api/ping', async () => jsonResponse({ ok: true, service: 'koyze-api' })],
