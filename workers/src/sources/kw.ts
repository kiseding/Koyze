...
export async function importKw(id: string) {
  ...
  return {
    ...
    songs: (data.musiclist || []).map((s: any) => {
      ...
      q.reverse();
      const img = s.pic || s.img || s.img300 || (s.web_albumpic_short
        ? `https://img1.kuwo.cn/star/albumcover/${s.web_albumpic_short}`
        : (s.web_artistpic_short
          ? `https://img1.kuwo.cn/star/starheads/${s.web_artistpic_short.replace(/^120\//, '500/')}`
          : ''));
      return { name: s.name, singer: s.artist, source: 'kw', songmid: s.rid ? `${s.rid}` : s.id, interval: `${s.duration || 0}`, img, albumName: s.album || '', types: q };
    }),
  };
}
