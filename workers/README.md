# koyze-api（Cloudflare Workers）

给 **Koyze** 用的精简云端：账号、管理员、云端歌单、歌单导入。**不负责** 搜索 / 播放 URL / 歌词（App 本地 + 用户导入的自定义源完成）。
...
App 路径：**设置 → 云端账号 / 歌单**，填写 `https://koyze-api.<account>.workers.dev`。
...
- **本地**：`npx wrangler d1 migrations apply koyze-api --local`
- **生产**（在 Worker 部署前）：`npx wrangler d1 migrations apply koyze-api --remote`  
