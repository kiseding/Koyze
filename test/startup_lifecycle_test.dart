...
void main() {
  test('registered resources dispose sequentially in reverse order', () async {
    ...
    expect(events, ['dependent-start', 'dynamic-resource', 'dependency-start']);
  });

  test(
    'audio runtime disposes subscriptions before handler before cache',
    () async {
      final events = <String>[];
      final tracker = ResourceDisposalTracker();
  ...
  test(
    'overlapping direct drains share completion failure and allow reuse',
    () async {
  ...
  test(
    'dispose drains resources registered synchronously by a disposer',
    () async {
  ...
  test(
    'dispose reaches a fixed point before and after container teardown',
    () async {
      final events = <String>[];
      final tracker = ResourceDisposalTracker();
      tracker.register(() async {
  ...
  test(
    'synchronous disposer failure does not stop remaining cleanup',
    () async {
  ...
  test(
    'asynchronous disposer failure does not stop callback-started cleanup',
    () async {
  ...
  test(
    'repeated dispose shares one drain and reports failure after cleanup',
    () async {
      final releaseCleanup = Completer<void>();
  ...
  test(
    'startup failure disposes container and drains tracked resources once',
    () async {
  ...
  test(
    'production provider graph drains async services before dependencies',
    () async {
      ...
      expect(
        events,
        containsAll(<String>['playlist', 'download', 'custom-source']),
      );
      expect(
        events.indexOf('playlist'),
        lessThan(events.indexOf('custom-source')),
      );
      expect(
        events.indexOf('download'),
        lessThan(events.indexOf('custom-source')),
      );
