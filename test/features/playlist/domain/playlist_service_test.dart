...
void main() {
  test(
    'same raw id from different sources remains independently addressable',
    () async {
      ...
      expect(await service.addSongToPlaylist('favorites', tx), isTrue);
      expect(await service.addSongToPlaylist('favorites', wy), isTrue);
      expect(service.favorites!.songs, hasLength(2));
      expect(await service.removeSongFromPlaylist('favorites', tx.id), isFalse);
      expect(
        await service.removeSongFromPlaylist('favorites', tx.identityKey),
        isTrue,
      );
      expect(service.favorites!.songs.single, wy);
      await service.dispose();
    },
  );

  test('search ranks title matches before singer and album matches', () async {
    final titleMatch = detailedSong('title', name: '情歌');
    final singerMatch = detailedSong(
      'singer',
      name: '晴天',
    ).copyWith(singer: '情歌乐队');
    final albumMatch = detailedSong(
      'album',
      name: '夜曲',
    ).copyWith(album: '情歌精选');
    final repository = MemoryPlaylistRepository(
      systemSnapshot(
        additional: [
          playlist('custom', songs: [albumMatch, singerMatch, titleMatch]),
        ],
      ),
    );
    final service = PlaylistService(repository: repository);
    ...
    final matches = await service.searchSongs('情歌');

    expect(matches.map((match) => match.song.id).toList(), [
      'title',
      'singer',
      'album',
    ]);
  ...
  test('recent trims legacy entries above 100 during initialization', () async {
    final oldSongs = [for (var index = 0; index < 101; index++) song('$index')];
    final repository = MemoryPlaylistRepository(
      PlaylistSnapshot(
        schemaVersion: 1,
        playlists: [
    ...
    expect(service.recent!.songs, hasLength(100));
    expect(service.recent!.songs.last.id, '99');
    expect(
      repository.saves.single.playlists
          .firstWhere((item) => item.id == 'recent')
          .songs,
  ...
  test('new favorites are inserted at the front', () async {
    final repository = MemoryPlaylistRepository(
      systemSnapshot(
        additional: [
          playlist('source', songs: [song('new')]),
        ],
      ),
    );
    final service = PlaylistService(repository: repository, clock: () => _now);
    await service.init();
    await service.addSongToPlaylist('favorites', song('old'));

    expect(await service.addSongToPlaylist('favorites', song('new')), isTrue);
    expect(service.getPlaylist('favorites')!.songs.map((item) => item.id), [
      'new',
      'old',
    ]);
    await service.dispose();
  ...
  test('bulk favorites are inserted at the front in source order', () async {
    final repository = MemoryPlaylistRepository(
      systemSnapshot(
        additional: [
          playlist('source', songs: [song('new-a'), song('new-b')]),
        ],
      ),
    );
    final service = PlaylistService(repository: repository, clock: () => _now);
  ...
  test('mutation invoked during init waits for loaded state', () async {
    ...
    expect(
      repository.saves.single.playlists.map((item) => item.id),
      containsAll(['favorites', 'recent', 'local', 'loaded', 'created']),
    );
  ...
  test(
    'local playlist mutations are recorded after durable local writes',
    () async {
      ...
      final service = PlaylistService(
        repository: repository,
        syncRecorder:
            ({required eventType, required entityId, required payload}) async {
              events.add({
                'eventType': eventType,
                'entityId': entityId,
                'payload': payload,
                'saved': repository.saves.isNotEmpty,
              });
            },
      );
      await service.init();

      await service.addSongToPlaylist('favorites', detailedSong('saved'));

      expect(events.single['eventType'], 'favorite.add');
      expect(events.single['saved'], isTrue);
      await service.dispose();
    },
  );
}
