...
void main() {
  group('预设曲线', () {
    ...
    test('控制点处取到预设值', () {
      ...
      expect(bass.gainsFor(_fiveBands), <double>[6, 4, 1, 0, 0]);
    });

    test('相邻控制点之间按对数频率线性插值', () {
      final bass = equalizerPresetById('bass')!;
      // 60Hz(6dB) 与 230Hz(4dB) 的几何中点上应恰好取到 5dB。
      final geometricMean = math.sqrt(60 * 230);
      expect(bass.gainAt(geometricMean), closeTo(5, 1e-9));
      // 线性频率轴的 145Hz 在对数轴上偏高频一侧，取值必须低于线性插值的 5dB。
      expect(bass.gainAt((60 + 230) / 2), lessThan(5));
    });

    test('两端之外取端点值', () {
      final treble = equalizerPresetById('treble')!;
      expect(treble.gainAt(10), 0); // 低于最低控制点 60Hz
      expect(treble.gainAt(24000), 6); // 高于最高控制点 14kHz
      expect(treble.gainAt(0), 0); // 非法频率不抛异常
    });
    ...
    test('同一曲线可采样到 5 段与 10 段两种布局', () {
      ...
      expect(rock.gainsFor(_fiveBands).first, closeTo(4, 0.5));
      expect(rock.gainsFor(_tenBands).first, closeTo(4, 0.5));
    });
  ...
  group('EqualizerSettings', () {
    ...
    test('段数不一致时回落到预设曲线', () {
      ...
      expect(gains.first, closeTo(6, 0.5));
      expect(gains.last, closeTo(0, 0.001));
    });
