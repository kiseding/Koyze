...
import 'package:koyze/features/home/presentation/main_scaffold.dart';
...
void main() {
  ...
  testWidgets('swipe right on first page wraps to last page', (tester) async {
    var selected = 0;
    await tester.pumpWidget(
      MaterialApp(
        home: MediaQuery(
          ...
          child: StatefulBuilder(
            builder: (context, setState) {
              return SwipeBranchContainer(
                ...
                onSelect: (i) => setState(() => selected = i),
                children: const [
                  ColoredBox(key: Key('home'), color: Colors.red),
                  ColoredBox(key: Key('second'), color: Colors.orange),
                  ColoredBox(key: Key('third'), color: Colors.green),
                  ColoredBox(key: Key('settings'), color: Colors.blue),
                ],
              );
            },
          ),
    ...
    await tester.drag(find.byKey(const Key('home')), const Offset(240, 0));
    await tester.pumpAndSettle();
    expect(selected, 3);
  });

  testWidgets('swipe left on last page wraps to first page', (tester) async {
    var selected = 3;
    await tester.pumpWidget(
      MaterialApp(
        home: MediaQuery(
    ...
    await tester.drag(find.byKey(const Key('settings')), const Offset(-240, 0));
    await tester.pumpAndSettle();
    expect(selected, 0);
  });
}
