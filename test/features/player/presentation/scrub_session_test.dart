...
import 'package:koyze/features/player/presentation/scrub_session.dart';
...
void main() {
  ...
  test(
    'current completion reports whether widget state may be cleared',
    () async {
      final session = ScrubSession(
        begin: () async => 9,
        finish: (_, __, {required resumeAfter}) async {},
  ...
  test(
    'begin cancels a superseded operation without its continuation',
    () async {
      final firstBegin = Completer<int>();
      final secondBegin = Completer<int>();
      final begins = <Completer<int>>[firstBegin, secondBegin];
  ...
  test(
    'failed begin is reported once without escaping and session continues',
    () async {
  ...
  test(
    'failed cancel is reported once without escaping and dispose stays safe',
    () async {
      final reports = <Object>[];
      final escaped = <Object>[];
  ...
  test(
    'default error handler captures construction zone and reports once',
    () async {
