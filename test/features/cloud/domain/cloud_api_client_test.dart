...
void main() {
  ...
  test('adds favorites in byte-bounded authenticated batches', () async {
    SharedPreferences.setMockInitialValues({
      'cloud_api_base': 'https://cloud.example',
    });
    final requests = <RequestOptions>[];
    ...
    expect(
      requests.every(
        (request) => request.path == 'https://cloud.example/api/user/love/add',
      ),
      isTrue,
    );
    expect(
      requests.expand((request) => (request.data as Map)['songs'] as List),
      hasLength(501),
    );
    expect(
      requests.every(
  ...
  test(
    'splits metadata-heavy favorite payloads below the worker cap',
    () async {
      SharedPreferences.setMockInitialValues({
        'cloud_api_base': 'https://cloud.example',
      });
  ...
  test('appends playlist songs in byte-bounded mode append requests', () async {
    SharedPreferences.setMockInitialValues({
      'cloud_api_base': 'https://cloud.example',
    });
    final requests = <RequestOptions>[];
    ...
    await testClient.appendPlaylistSongs(
      id: 'road-trip',
      name: 'Road Trip',
      position: 1,
      songs: [
    ...
    expect(requests.length, greaterThan(1));
    for (final request in requests) {
      expect(request.path, 'https://cloud.example/api/user/list');
      final entry = ((request.data as Map)['userList'] as List).single;
      expect(entry['id'], 'road-trip');
  ...
  test('stages and commits a replacement snapshot by revision', () async {
          ...
          onRequest: (options, handler) {
            ...
            handler.resolve(
              Response(
                requestOptions: options,
                statusCode: 200,
                data: options.path.endsWith('/commit')
                    ? const {'ok': true, 'revision': 12}
                    : const {'ok': true},
              ),
            );
          },
    ...
    final revision = await client.replaceCloudSnapshot(
      loveList: const [],
      userList: const [
        {'id': 'road', 'name': 'Road', 'list': []},
      ],
      baseRevision: 11,
      expectedSessionRevision: client.sessionRevision,
    );

    expect(revision, 12);
    expect(requests, hasLength(3));
    expect(requests[0].path, endsWith('/api/user/sync/stage'));
    expect(requests[1].path, endsWith('/api/user/sync/stage'));
    expect(requests[2].path, endsWith('/api/user/sync/commit'));
    expect((requests[2].data as Map)['baseRevision'], 11);
    expect((requests[2].data as Map)['manifest'], [
      {'id': 'love', 'count': 0},
      {'id': 'road', 'count': 0},
    ]);
  });
