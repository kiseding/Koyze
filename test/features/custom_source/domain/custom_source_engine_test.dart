...
import 'package:koyze/features/custom_source/domain/source_runtime_polyfill.dart';
...
void main() {
  ...
  test('flush drives the QuickJS pending job queue', () {
    final bridge = File(
      'lib/features/custom_source/domain/custom_source_engine.dart',
    ).readAsStringSync();
    final flushBlock = RegExp(
      r'void _flushMicrotasks\(\{int maxIterations = 64\}\) \{([\s\S]*?)\n  \}',
    ).firstMatch(bridge)!.group(1)!;

    expect(flushBlock, contains('_runtime!.evaluate'));
    expect(flushBlock, contains('_runtime!.executePendingJob'));
  });
