...
void main() {
  ...
  group('LocalMusicLibrary', () {
    ...
    test('scraped identity replaces filename fallback in app index', () async {
      final storage = StorageService.forTesting(
        await SharedPreferences.getInstance(),
      );
      ...
      expect(library.files[path]?['title'], 'Scraped Title');
      expect(library.files[path]?['artist'], 'Scraped Artist');
      expect(library.files[path]?['album'], 'Scraped Album');
    });
  });
