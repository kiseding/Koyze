...
void main() {
  ...
  test(
    'promote part must not delete .part before rename (filesystem)',
    () async {
      ...
      const base = 'song123';
      final part = File.fromUri(dir.uri.resolve('$base.part'));
      final finalPath = dir.uri.resolve('$base.mp3').toFilePath();
      // 模拟旧 bug：cleanup 删掉 .part
      await part.writeAsBytes(
        List<int>.filled(4096, 1)..setAll(0, [0x49, 0x44, 0x33]),
      );
  ...
  test('attempt revision defaults to zero and survives persistence', () {
    ...
    expect(
      DownloadTask.fromJson(
        task.copyWith(attemptRevision: 4).toJson(),
      ).attemptRevision,
      4,
    );
  });
  ...
  test('isPlayableMediaUrl leaves media content validation to downloader', () {
    expect(
      isPlayableMediaUrl('https://media.example.com/?token=signed'),
      isTrue,
    );
    expect(
  ...
  test(
    'fresh-link download resolves once when the first link succeeds',
    () async {
      var resolveCalls = 0;
      final result = const PlayUrlResult(
  ...
  test(
    'fresh-link download re-resolves only after an expired HTTP response',
    () async {
      var resolveCalls = 0;
      var downloadCalls = 0;
  ...
  test(
    'fresh-link download does not re-resolve after another HTTP failure',
    () async {
      var resolveCalls = 0;
  ...
  test(
    'cancellation after expired response prevents fresh re-resolve',
    () async {
  ...
  test(
    'fresh resolver rejects pre-cancellation instead of returning null',
    () async {
      final token = CancelToken()..cancel('paused');
      var serviceCalls = 0;
  ...
  test(
    'fresh resolver cancellation wins over simultaneous resolver error',
    () async {
      final token = CancelToken();
  ...
  test(
    'download cancellation wins over simultaneous non-expired HTTP error',
    () async {
      final token = CancelToken();
