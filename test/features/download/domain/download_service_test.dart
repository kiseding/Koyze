...
void main() {
  ...
  test(
    'same raw music id from different sources creates separate tasks',
    () async {
      var nextId = 0;
      final service = DownloadService(
        wifiOnly: true,
        ...
        storage: _MemoryStorage(),
        taskIdFactory: () => 'task-${nextId++}',
      );
      addTearDown(service.dispose);
      final tx = _song('shared');
      final wy = MusicItem(
        id: 'shared',
        name: 'WY',
        singer: 'Singer',
      ...
      await service.addTasks([tx, wy]);

      expect(service.tasks, hasLength(2));
      expect(service.tasks.map((task) => task.musicIdentityKey).toSet(), {
        tx.identityKey,
        wy.identityKey,
      });
    },
  ...
  test(
    'download task persistence discards legacy and newly resolved URLs',
    () async {
      ...
      ).toJson()..['url'] = 'https://media.example/play?token=private';
      final decoded = DownloadTask.decodePersisted(legacy);

      expect(decoded.url, isNull);
      expect(decoded.toJson(), isNot(contains('url')));
    },
  );
  ...
  test('startup drops completed records for missing and empty files', () async {
    final root = await Directory.systemTemp.createTemp('downloads_');
    final empty = File('${root.path}/Empty-empty-1.mp3')
    ...
    final storage = _MemoryStorage()
      ..saved = [
        DownloadTask(
          ...
          createdAt: DateTime.utc(2026),
          status: DownloadStatus.completed,
          savePath: '${root.path}/Missing-missing-1.mp3',
        ).toJson(),
        DownloadTask(
          id: 'empty',
          musicId: 'm2',
  ...
  test(
    ...
    () async {
      final root = await Directory.systemTemp.createTemp('downloads_');
      final nonEmpty = File('${root.path}/Song-task-2.mp3')
        ..writeAsBytesSync(List<int>.filled(2048, 1));
      final empty = File('${root.path}/Song-task-2.flac')
        ..writeAsBytesSync(const []);
      final unrelated = File('${root.path}/notes.txt')
        ..writeAsStringSync('keep');
      final storage = _MemoryStorage()
  ...
  test(
    ...
    () async {
      final root = await Directory.systemTemp.createTemp('downloads_');
      final emptyMp3 = File('${root.path}/Song-task-2.mp3')
        ..writeAsBytesSync(const []);
      final emptyFlac = File('${root.path}/Song-task-2.flac')
        ..writeAsBytesSync(const []);
      final unrelated = File('${root.path}/notes.txt')
        ..writeAsStringSync('keep');
      final storage = _MemoryStorage()
