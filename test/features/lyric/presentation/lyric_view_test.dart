...
void main() {
  testWidgets(
    'shows loading state while the current lyric request is pending',
    (tester) async {
  ...
  testWidgets(
    'shows safe actionable error without exposing exception details',
    (tester) async {
      await _pumpView(
        tester,
...
Future<void> _pumpView(WidgetTester tester, LyricLoader loader) async {
  await tester.pumpWidget(
    ProviderScope(
      ...
      child: const MaterialApp(home: Scaffold(body: LyricView())),
    ),
  );
}
