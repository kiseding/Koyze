...
void main() {
  ...
  test('five failed tracks halt auto-skip until explicit next', () async {
    ...
    await handler.setPlaylist(const [
      ...
      MediaItem(id: 'B', title: 'B'),
      MediaItem(id: 'C', title: 'C'),
      MediaItem(id: 'D', title: 'D'),
      MediaItem(id: 'E', title: 'E'),
      MediaItem(id: 'F', title: 'F'),
    ]);

    expect(resolved, ['A', 'B', 'C', 'D', 'E']);
    expect(handler.currentQueueIndex, 4);
    expect(handler.debugConsecutiveAutomaticTrackFailures, 5);
    expect(handler.debugAutomaticFailureSkippingHalted, isTrue);
    expect(handler.playbackState.value.playing, isFalse);

    handler.urlResolver = (id, [extras]) async => 'file:///tmp/$id.mp3';
    await handler.skipToNext();

    expect(handler.currentQueueIndex, 5);
    expect(handler.mediaItem.value?.id, 'F');
    expect(handler.debugConsecutiveAutomaticTrackFailures, 0);
    expect(handler.debugAutomaticFailureSkippingHalted, isFalse);
    expect(player.playing, isTrue);
  });

  test('explicit play retries the fifth failed track', () async {
    final player = _PlaybackStateAudioPlayer()
      ..sourceInstallProcessingState = ProcessingState.ready;
    final handler = LxAudioHandler(player: player);
    addTearDown(player.dispose);
    ...
    expect(handler.mediaItem.value?.id, 'E');
    expect(handler.debugAutomaticFailureSkippingHalted, isTrue);
  ...
  test('explicit previous resets a halted failure chain', () async {
    final player = _PlaybackStateAudioPlayer()
      ..sourceInstallProcessingState = ProcessingState.ready;
    final handler = LxAudioHandler(player: player);
    addTearDown(player.dispose);
    ...
    expect(handler.mediaItem.value?.id, 'D');
    expect(handler.debugConsecutiveAutomaticTrackFailures, 0);
    expect(handler.debugAutomaticFailureSkippingHalted, isFalse);
    expect(player.playing, isTrue);
  });

  test('direct queue selection resets a halted failure chain', () async {
    final player = _PlaybackStateAudioPlayer()
      ..sourceInstallProcessingState = ProcessingState.ready;
    final handler = LxAudioHandler(player: player);
    addTearDown(player.dispose);
  ...
  test('repeated native play failures halt on the fifth track', () async {
    final player = _PlaybackStateAudioPlayer()
      ..sourceInstallProcessingState = ProcessingState.ready;
    for (var i = 0; i < 10; i++) {
      final failure = player.gateNextPlayFailure();
      failure.release.complete();
    }
    final handler = LxAudioHandler(player: player);
    addTearDown(player.dispose);
    handler.urlResolver = (id, [extras]) async => 'file:///tmp/$id.mp3';
    ...
    for (var i = 0; i < 30; i++) {
      if (handler.debugAutomaticFailureSkippingHalted) break;
      await pumpEventQueue();
    }
