...
void main() {
  test(
    'dispose cancels listeners then drains started callback before owners',
    () async {
      final events = <String>[];
      ...
      expect(events, ['callback-start', 'interruption-cancel', 'noisy-cancel']);
  ...
  test(
    'dispose continues after listener callback handler and cache failures',
    () async {
      ...
      runtime.attachAudioSession<int>(
        interruptionEvents: _CancelFailingStream(
  ...
  test(
    'dispose reports a callback failure that completed before teardown',
    () async {
      final callbackFailure = StateError('callback');
      final interruptions = StreamController<int>.broadcast();
      final noisy = StreamController<void>.broadcast();
  ...
  test(
    'startup failure after session wiring drains its started callback',
    () async {
  ...
  test(
    'partial session wiring owns the first successful subscription',
    () async {
