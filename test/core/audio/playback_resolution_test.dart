  ...
  group('PlaybackUrlResolver', () {
    test('denied QQ HTTP media never reaches the native player', () async {
      ...
      final resolver = PlaybackUrlResolver<MusicItem>(
        ...
        validateStream:
            ({required remoteUrl, required platform, required quality}) async {
              validationCalls++;
              return false;
            },
    ...
    test(
      'not-found media skips stream validation and tries lower quality',
      () async {
    ...
    test(
      'validated HTTP media can stream when cache transport is blocked',
      () async {
        final resolver = PlaybackUrlResolver<MusicItem>(
          resolvePlayableUrl: (music, {required preferredQuality}) async =>
              _playResult(url: 'http://nf.sycdn.kuwo.cn/a.mp3', platform: 'kw'),
          acquireOrDownload:
              ({
                required remoteUrl,
                required platform,
      ...
      final resolver = PlaybackUrlResolver<MusicItem>(
        ...
        acquireOrDownload:
            ({
              ...
              required songId,
              required quality,
            }) async => fail('cache must not run without remote'),
        songIdFor: (music) => music.songmid ?? music.id,
      );
  ...
  group('LxAudioHandler cached file reuse', () {
    test(
      ...
      () async {
        ...
        expect(acquired, [Uri.file(lease.path).toFilePath()]);
    ...
    test(
      ...
      () async {
        ...
        handler.attachPlaybackCache(
          acquireExisting: (path) async =>
              path == Uri.file(old.path).toFilePath()
              ? old.asLease()
              : replacement.asLease(),
        );
    ...
    test(
      ...
      () async {
        ...
        handler.attachPlaybackCache(
          classifyExisting: (path) async {
            classifyCalls++;
            expect(path, Uri.file('/cache/new.mp3').toFilePath());
            return const RejectedPlaybackCachePath();
          },
        );
        handler.urlResolver = (id, [extras]) async {
