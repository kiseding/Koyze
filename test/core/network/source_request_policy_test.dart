...
void main() {
  ...
  SourceRequestPolicy policyWith(
  ...
  group('SourceRequestPolicy', () {
    ...
    test(
      'rejects credentials, missing hosts, and unsupported schemes',
      () async {
        final policy = policyWith({
          'example.com': ['93.184.216.34'],
        });
        ...
        final httpRequest = await policy.validate(
    ...
    test('rejects reserved and private IPv4 destinations', () async {
      final policy = policyWith({});
    ...
    test(
      'rejects every IPv4 special-purpose prefix at its boundaries',
      () async {
        final policy = policyWith({});
    ...
    test(
      'rejects every IPv6 special-purpose prefix at its boundaries',
      () async {
        final policy = policyWith({});
        const prefixes = <List<String>>[
          ['::', '::ffff:ffff'],
    ...
    test('prefers IPv4 when both public families are returned', () async {
      ...
      final policy = policyWith({
        'dual.example': ['2606:2800:220:1:248:1893:25c8:1946', '93.184.216.34'],
      });

      final request = await policy.validate(
        Uri.parse('https://dual.example/a'),
        {},
      );
    ...
    test(
      'rejects DNS results containing only private IPv4 addresses',
      () async {
        final policy = policyWith({
    ...
    test('clamps timeout to the supported range', () async {
      ...
      final short = await policy.validate(Uri.parse('https://example.com'), {
        'timeout': -1,
      });
      final long = await policy.validate(Uri.parse('https://example.com'), {
        'timeout': 999999,
      });
  ...
  group('SourceRequestSandbox', () {
    Future<List<ValidatedSourceRequest>> followRedirect({
      ...
      await sandbox.request(Uri.parse('https://one.example/start'), {
        'method': method,
        'body': body,
        'headers':
            headers ??
    ...
    for (final status in [301, 302]) {
      test(
        '$status rewrites POST to GET and drops body content headers',
        () async {
    ...
    test(
      'cross-origin body-dropping redirect drops body content headers',
      () async {
        final requests = await followRedirect(
          status: 302,
          location: 'https://two.example/done',
    ...
    test(
      'validates every redirect and passes validated addresses to transport',
      () async {
    ...
    test(
      'pre-cancelled request does not resolve or invoke transport',
      () async {
        var resolverCalls = 0;
        var transportCalls = 0;
    ...
    test('bounds a stalled transport by the validated timeout', () async {
      ...
      await expectLater(
        sandbox.request(Uri.parse('https://example.com'), {'timeout': 1}),
        throwsA(
          isA<SourceRequestPolicyException>().having(
            (error) => error.code,
            'code',
    ...
    test('rejects aggregate in-flight bytes and releases budget', () async {
      ...
      final afterRelease = await sandbox.request(
        Uri.parse('https://example.com/three'),
        {},
      );
    ...
    test(
      'rejects excess concurrent response bodies and releases the slot',
      () async {
        final firstBody = StreamController<List<int>>();
        var calls = 0;
        final sandbox = SourceRequestSandbox(
    ...
    test(
      'acquires request admission before DNS and transfers it to response',
      () async {
        var resolverCalls = 0;
        final sandbox = SourceRequestSandbox(
          policy: SourceRequestPolicy(
        ...
        final first = await sandbox.request(
        ...
        first.release();
        first.release();
        final second = await sandbox.request(
    ...
    test('request errors release admission', () async {
      ...
      fail = false;
      final response = await sandbox.request(
        Uri.parse('https://example.com/ok'),
        {},
      );
      response.release();
    });
    ...
    test(
      'cancellation after body completion releases bytes and admission',
      () async {
        final cancellation = SourceRequestCancellation();
        var calls = 0;
        final sandbox = SourceRequestSandbox(
        ...
        final next = await sandbox.request(
          Uri.parse('https://example.com/two'),
          {},
        );
        expect(next.bytes, [2]);
    ...
    test(
      'callback gate holds permits and bytes until callback finishes',
      () async {
        ...
        final third = await sandbox.request(
          Uri.parse('https://example.com/three'),
          {},
        );
        third.release();
    ...
    test(
      'cancellation during delivery retains ownership until gate exits',
      () async {
        final cancellation = SourceRequestCancellation();
        ...
        final delivery = withSourceResponseLease(response, (
          ownedResponse,
        ) async {
          entered.complete();
          await gate.future;
    ...
    test('redirect setup failures close response exactly once', () async {
      for (final testCase in <({String name, String location, dynamic body})>[
        ...
        (name: 'stream', location: '/next', body: Stream.value([1])),
      ]) {
        var closes = 0;
        final sandbox = SourceRequestSandbox(
          policy: policyWith({
