...
import 'package:koyze/core/music_source/platform/tx_source.dart';

void main() {
  test('Tx exact filename candidates never contain a lower quality prefix', () {
    expect(TxSource.exactFilenames('song', 'media', 'flac'), [
      'F000media.flac',
      'F000song.flac',
    ]);
  ...
  test(
    'Tx exact attempt keys collapse aliases and reject unsupported quality',
    () {
  ...
  test(
    'Tx legacy unknown quality keeps the historical fallback candidates',
    () {
      expect(TxSource.legacyFilenames('song', 'media', 'future-quality'), [
        'C400song.m4a',
        'M500song.mp3',
      ]);
    },
