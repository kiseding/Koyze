...
private let widgetGroupId = "group.com.koyze.app"
...
private struct LXNowPlayingWidgetView: View {
  ...
  var body: some View {
    ...
    .widgetURL(URL(string: "koyze://nowplaying"))
  }
}

struct KoyzeHomeWidget: Widget {
  let kind = "KoyzeHomeWidget"
...
private func lxDynamicIsland(context: ActivityViewContext<LiveActivitiesAppAttributes>) -> DynamicIsland {
  ...
  let title = context.state.title ?? (value("title") ?? "Koyze")
...
@available(iOSApplicationExtension 16.1, *)
struct KoyzeLiveActivity: Widget {
  var body: some WidgetConfiguration {
...
@main
struct KoyzeWidgets: WidgetBundle {
